...
void printCourseList(const AVLTree& tree) {
    if (tree.empty()) {
...
void printTreeMetrics(const AVLTree& tree) {
    if (tree.empty()) {
        std::cout << "Please load the data structure first (Option 1).\n";
        return;
    }

    std::cout << "Courses stored: " << tree.size() << '\n'
              << "Tree height: " << tree.height() << '\n'
              << "AVL balance check: "
              << (tree.isBalanced() ? "passed" : "failed") << '\n';
}
    ...
    while (choice != 9) {
        ...
        case 1: {
            ...
            if (result.success) {
                ...
                std::cout << ".\n";
                std::cout << "Balanced AVL height: " << tree.height() << ".\n";
            }
            break;
        }
